resource "aws_rds_cluster_parameter_group" "rds_cluster_parameter_group" {
  count       = var.rds_cluster_parameter_group_name == null && var.rds_cluster_parameter_group_name_prefix == null ? 0 : 1
  ...
  dynamic "parameter" {
    for_each = var.rds_cluster_parameter_group_parameter == {} ? [] : [var.rds_cluster_parameter_group_parameter]
    content {
